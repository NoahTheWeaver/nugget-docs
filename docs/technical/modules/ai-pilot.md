...
**Status (August 2026):** installed and configured on the sandbox; pilot roster is Noah, Bo and Gabrielle. Not yet on production. Earlier plan for this page (Gemini + Wyatt docs as a knowledge source, no custom code) is superseded by what follows.
...
- `nugget_ai_claude` patches `odoo.addons.ai.utils.llm_api_service.LLMApiService` and appends to `odoo.addons.ai.utils.llm_providers.PROVIDERS`. Re-verify on every Odoo update: the `Provider` tuple already grew a field (`deprecated_models`) between our local checkout and the Odoo.sh build, which is why the registration is built from `Provider._fields` and every override forwards `*args/**kwargs`.
